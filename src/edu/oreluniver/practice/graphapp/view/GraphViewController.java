...
public class GraphViewController {
  ...
  private void initialize(){
    ...
    panner.setMouseFilter(mouseEvent -> {
      updateAxisGrid();
      if (mouseEvent.getButton() == MouseButton.PRIMARY && mainViewController.getCurrentDot() == null) {
      } else {
        mouseEvent.consume();
      }
    });
    ...
    TableViewController.setPattern(leftBoundTextField, rightBoundTextField);
  ...
  private void updateChartOnClickListener(Node node){
    ...
    node.setOnMouseClicked(event -> {
      ...
      mainViewController.setCurrentDot(chosenDot);
  ...
  private void updateChartOnDraggedListener(Node node, Data<Double, Double> data){
    ...
    node.setOnMouseDragged(e -> {
      ...
      mainViewController.getCurrentDot().setPosX(x);
      mainViewController.getCurrentDot().setPosY(y);

      updateCurrentDotLabel();
      mainViewController.getTableController().updateTable();
//        controller.getTableController().debugDots();
  ...
  public void updateGraph(){
    ...
    List<Dot> dots = DotsController.getInstance().getDotList();
  ...
  public void updateCurrentDotLabel(){
    Dot dot = mainViewController.getCurrentDot();
    if (dot == null) {
  ...
  public void deleteCurrentDot() throws Exception {
    ...
    mainViewController.deleteCurrentDot();
  ...
  public void setBounds(){
    ...
    if (mainViewController.isFieldEmpty(leftBoundTextField))
      leftValue = - Double.MAX_VALUE;
    else
      leftValue = Double.parseDouble(leftBoundTextField.getText());

    if (mainViewController.isFieldEmpty(rightBoundTextField))
    ...
    if (rightValue < leftValue) {
      mainViewController.markWrongFields(leftBoundTextField, rightBoundTextField);
      return;
    }
